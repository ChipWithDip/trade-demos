...
/* glass template: lightbox for the masonry gallery (native <dialog>, keyboard friendly) */
(function () {
    'use strict';
    var lb = document.getElementById('lightbox');
    if (!lb || typeof lb.showModal !== 'function') return;
    var img = lb.querySelector('img');
    var links = Array.prototype.slice.call(document.querySelectorAll('[data-light]'));
    ...
    function nav(dir, label, text) {
        var b = document.createElement('button');
        b.type = 'button';
        b.className = 'lb-nav lb-nav--' + dir;
        b.setAttribute('aria-label', label);
        b.textContent = text;
        b.addEventListener('click', function () { show(current + (dir === 'prev' ? -1 : 1)); });
        lb.appendChild(b);
        return b;
    }
    nav('prev', 'Previous photo', '\u2039');
    nav('next', 'Next photo', '\u203A');
    // On document, not the dialog: clicking the photo can move focus off the dialog (to <body> in some browsers), which would stop the keys.
    document.addEventListener('keydown', function (e) {
        if (!lb.open) return;
        if (e.key === 'ArrowLeft') { e.preventDefault(); show(current - 1); }
        else if (e.key === 'ArrowRight') { e.preventDefault(); show(current + 1); }
        else if (e.key === 'Escape') { e.preventDefault(); lb.close(); }
    });
